...
from legged_gym.envs.wk4sc.wk4sc_leg_config import Wk4scWPLocoCfg


class Wukong4scLegbase(BaseTask):
    gait_manager: GaitManager = None

    def __init__(self, cfg: Wk4scWPLocoCfg, sim_params, physics_engine, sim_device, headless):
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.enable_viewer_sync and self.debug_wp:
    ...
    def compute_observations(self):
        ...
        dist = (self.waypoint - self.root_states[:,:2]).norm(dim=1).unsqueeze(1)
        self.privileged_obs_buf = torch.cat((self.body_vel_buf, self.obs_buf, dist), dim=-1)
        ...
        # self.privileged_obs_buf = torch.cat((self.body_vel_buf, self.obs_buf, self.base_height_buf, self.foot_heightmap_buf), dim=-1)
        ...
        # The latest observations are concat at the end of the history buffer
        # self.obs_history[:,0] is the oldest
        # self.obs_history[:,-self.num_obs:] is the newest observation, and the same as self.obs_buf
    ...
    def _post_physics_step_callback(self):
        ...
        """
        # arrive_flag = (self.root_states[:, :2] - self.waypoint).norm(dim=1) < 0.05
        ...
        if self.cfg.commands.heading_command:
            ...
            self.commands[:, 2] = torch.clip(1.0 * wrap_to_pi(self.commands[:, 3] - heading),
    ...
    def _compute_torques(self, actions):
        ...
        if self.rand_joint_indices is not None:
            self.joint_pos_target[:, self.rand_joint_indices] += self.arm_dof_pos_rand
    ...
    def _resample_waypoint_commands(self, env_ids):
        ...
        stay_prop = torch.rand(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False) < 0.2
        stay_prop = stay_prop[env_ids]
        stay_idx = stay_prop.nonzero(as_tuple=False).flatten()
        resample_idx = (~stay_prop).nonzero(as_tuple=False).flatten()
    ...
    def _resample_commands(self, env_ids):
        ...
        self.commands[env_ids, 0] = torch.rand(len(env_ids), dtype=torch.float, device=self.device, requires_grad=False) * (self.cmd_range[env_ids, 1]-self.cmd_range[env_ids, 0]) + self.cmd_range[env_ids, 0]
        self.commands[env_ids, 1] = torch.rand(len(env_ids), dtype=torch.float, device=self.device, requires_grad=False) * (self.cmd_range[env_ids, 3]-self.cmd_range[env_ids, 2]) + self.cmd_range[env_ids, 2]
    ...
    def _reset_dofs(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_init_state:
            self.dof_pos[env_ids] = self.default_dof_pos * torch_rand_float(0.8, 1.2, (len(env_ids), self.num_dof),
                                                                            device=self.device)
            # meshgrid_id = torch.meshgrid(env_ids, self.rand_joint_indices)
            # self.dof_pos[meshgrid_id] += self.arm_dof_pos_rand[env_ids]
        else:
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_init_state:
            ...
            self.root_states[env_ids, 7:13] = torch_rand_float(-0.2, 0.2, (len(env_ids), 6),
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        if self.cfg.terrain.robot_move:
            distance = torch.norm(self.root_states[env_ids, :2] - self.robot_orgin[env_ids, :2], dim=1)
            # robots that walked far enough progress to harder terains
            move_up = distance > self.terrain.env_length / 2
            # move_up = distance > self.exp_dist_buf[env_ids]
            # robots that walked less than half of their required distance go to simpler terrains
            move_down = (distance < torch.norm(self.waypoint[env_ids]-self.robot_orgin[env_ids, :2], dim=1) * 0.5) * ~move_up
    ...
    def update_command_range_by_terrain(self, env_ids):
        ...
        if not self.init_done:
            # don't change on initial reset
            return
        self.cmd_range[env_ids,] = self.cmd_max - (self.cmd_max - self.cmd_min[self.terrain_types[env_ids]]) / (
                    self.cfg.terrain.num_cols - 1) * self.terrain_levels[env_ids].unsqueeze(1)
    ...
    def _init_buffers(self):
        ...
        self.cmd_range[:, ] = torch.tensor([self.command_ranges["lin_vel_x"][0], self.command_ranges["lin_vel_x"][1],
                                            self.command_ranges["lin_vel_y"][0], self.command_ranges["lin_vel_y"][0]],
                                           dtype=torch.float, device=self.device, requires_grad=False)
        self.cmd_max = torch.tensor([self.command_ranges["lin_vel_x"][0], self.command_ranges["lin_vel_x"][1],
                                     self.command_ranges["lin_vel_y"][0], self.command_ranges["lin_vel_y"][0]],
                                    dtype=torch.float, device=self.device, requires_grad=False)
        self.cmd_min = torch.zeros(self.cfg.terrain.num_cols, 4, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        ...
        self.cmd_min[int(self.cfg.terrain.num_cols * self.cfg.terrain.terrain_proportions[0]):, ] = (
            torch.tensor([-0.6, 0.8, -0.3, 0.3], dtype=torch.float, device=self.device, requires_grad=False))
        ...
        if hasattr(self.cfg.init_state, "rand_joint"):
            rand_joint = self.cfg.init_state.rand_joint
            self.rand_joint_indices = [self.dof_dict[name] for name in rand_joint]
            self.rand_joint_indices = torch.Tensor(data=self.rand_joint_indices).to(self.device).to(torch.long)
        else:
            self.rand_joint_indices = None
        print('rand_joint_indices:', self.rand_joint_indices)
        ...
        self.arm_dof_pos_rand = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device,
                                            requires_grad=False)
        self.arm_dof_pos_rand[:, :2] = torch_rand_float(self.cfg.init_state.shoulder_y_range[0],
                                                        self.cfg.init_state.shoulder_y_range[1], (self.num_envs, 2),
                                                        device=self.device)
        self.arm_dof_pos_rand[:, 2:] = torch_rand_float(self.cfg.init_state.elbow_range[0],
                                                        self.cfg.init_state.elbow_range[1], (self.num_envs, 2),
                                                        device=self.device)
    ...
    def _reward_cotr(self):
        ...
        return torch.clip(self.cot_vel - 0.8, min=0.)
